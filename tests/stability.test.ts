...
import {
  ...
  KORA_LONG_THRESHOLDS,
  LORA_LONG_THRESHOLDS,
  NORA_LONG_THRESHOLDS,
  MORA_LONG_THRESHOLDS,
  PORA_LONG_THRESHOLDS,
  QORA_LONG_THRESHOLDS,
  RORA_LONG_THRESHOLDS,
  SORA_LONG_THRESHOLDS,
  TORA_LONG_THRESHOLDS,
...
describe("стабильность деревни", () => {
  ...
  it("15120 дней, seed=2026 — двадцать один год, ремесло и демография", () => {
    const { report } = runModulation(15120, 2026, undefined, { thresholds: KORA_LONG_THRESHOLDS });
    assert.equal(
      report.stable,
      true,
      `15120d: ${report.issues.join("; ")} | alive=${report.finalAlive} births=${report.births} dead=${report.dead}`,
    );
    assert.ok(report.births >= 20, "ожидалось ≥20 рождений за ~21 год симуляции");
  });

  it("15840 дней, seed=2026 — двадцать два года, соль и долгий цикл", () => {
    const { report } = runModulation(15840, 2026, undefined, { thresholds: LORA_LONG_THRESHOLDS });
    assert.equal(
      report.stable,
      true,
      `15840d: ${report.issues.join("; ")} | alive=${report.finalAlive} births=${report.births} dead=${report.dead}`,
    );
    assert.ok(report.births >= 21, "ожидалось ≥21 рождений за ~22 года симуляции");
  });

  it("16560 дней, seed=2026 — двадцать три года, железо и стройка", () => {
    const { report } = runModulation(16560, 2026, undefined, { thresholds: NORA_LONG_THRESHOLDS });
    assert.equal(
      report.stable,
      true,
      `16560d: ${report.issues.join("; ")} | alive=${report.finalAlive} births=${report.births} dead=${report.dead}`,
    );
    assert.ok(report.births >= 22, "ожидалось ≥22 рождений за ~23 года симуляции");
  });

  it("17280 дней, seed=2026 — двадцать четыре года, казна и субсидии", () => {
    const { report } = runModulation(17280, 2026, undefined, { thresholds: MORA_LONG_THRESHOLDS });
    assert.equal(
      report.stable,
      true,
      `17280d: ${report.issues.join("; ")} | alive=${report.finalAlive} births=${report.births} dead=${report.dead}`,
    );
    assert.ok(report.births >= 23, "ожидалось ≥23 рождений за ~24 года симуляции");
  });

  it("18000 дней, seed=2026 — двадцать пять лет, лес и долгий цикл", () => {
    const { report } = runModulation(18000, 2026, undefined, { thresholds: PORA_LONG_THRESHOLDS });
    assert.equal(
      report.stable,
      true,
      `18000d: ${report.issues.join("; ")} | alive=${report.finalAlive} births=${report.births} dead=${report.dead}`,
    );
    assert.ok(report.births >= 24, "ожидалось ≥24 рождений за ~25 лет симуляции");
  });

  it("18720 дней, seed=2026 — двадцать шесть лет, истощение сил и долгий цикл", () => {
    const { report } = runModulation(18720, 2026, undefined, { thresholds: QORA_LONG_THRESHOLDS });
    assert.equal(
      report.stable,
      true,
      `18720d: ${report.issues.join("; ")} | alive=${report.finalAlive} births=${report.births} dead=${report.dead}`,
    );
    assert.ok(report.births >= 25, "ожидалось ≥25 рождений за ~26 лет симуляции");
  });

  it("19440 дней, seed=2026 — двадцать семь лет, перенаселение и долгий цикл", () => {
    const { report } = runModulation(19440, 2026, undefined, { thresholds: RORA_LONG_THRESHOLDS });
    assert.equal(
      report.stable,
      true,
      `19440d: ${report.issues.join("; ")} | alive=${report.finalAlive} births=${report.births} dead=${report.dead}`,
    );
    assert.ok(report.births >= 26, "ожидалось ≥26 рождений за ~27 лет симуляции");
  });

  it("20160 дней, seed=2026 — двадцать восемь лет, стройка и долгий цикл", () => {
    const { report } = runModulation(20160, 2026, undefined, { thresholds: SORA_LONG_THRESHOLDS });
    assert.equal(
      report.stable,
      true,
      `20160d: ${report.issues.join("; ")} | alive=${report.finalAlive} births=${report.births} dead=${report.dead}`,
    );
    assert.ok(report.births >= 27, "ожидалось ≥27 рождений за ~28 лет симуляции");
  });

  it("20880 дней, seed=2026 — двадцать девять лет, сторожа и долгий цикл", () => {
    const { report } = runModulation(20880, 2026, undefined, { thresholds: TORA_LONG_THRESHOLDS });
    assert.equal(
      report.stable,
      true,
      `20880d: ${report.issues.join("; ")} | alive=${report.finalAlive} births=${report.births} dead=${report.dead}`,
    );
    assert.ok(report.births >= 28, "ожидалось ≥28 рождений за ~29 лет симуляции");
  });
});
