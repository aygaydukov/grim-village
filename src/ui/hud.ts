...
export function refreshInspectorLive(selection: Selection, world: World): void {
  ...
  setText("live-v-unburied", String(r.unburiedBodies));
  const unburiedRow = elOptional("live-v-unburied-row");
  if (unburiedRow) unburiedRow.hidden = r.unburiedBodies <= 0;
  ...
  setText("live-v-sick-hut-count", String(r.sickHutCount));
