...
Обновлено: **2026-08-09** (ежедневный прогон Medieval Society Daily, v1.7.0).
...
|--------|---------|-------------|
| Path smoothing A* | **Сделано (v1.7.0)** | `smoothPath` + `hasLineOfSight` в `pathfind.ts`; меньше зигзагов. |
| Smoke 4320 дней | **Сделано (v1.7.0)** | `HYPER_LONG_THRESHOLDS`, ≥6 рождений, стабильно seed=2026. |
...
| Smoke 5040+ дней (7 лет) | **Следующий цикл** | Стресс после шести лет. |

## Следующие циклы (приоритет)

1. **Спрайты** — замена силуэтов.
2. **Smoke 5040 дней** — семь игровых лет.
3. **Отдельная «больная хижина»** — переселение заражённых в одну избу (сейчас изоляция по дому семьи).
...
| 4320 дней (v1.7.0) | HYPER_LONG, ≥6 рождений — **стабильно** |
| CI (`--ci`) 100 дней | 22+ живых, рождения | Стабильно по CI-порогам |

**Идеи для следующего цикла:** спрайты; smoke 5040 дней; центральная «больная изба».
