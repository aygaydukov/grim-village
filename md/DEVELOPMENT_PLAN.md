...
Обновлено: **2026-08-11** (ежедневный прогон Medieval Society Daily, v1.9.0).
...
| Smoke 5040 дней | **Сделано (v1.8.0)** | `OMEGA_LONG_THRESHOLDS`, ≥7 рождений. |
| Спрайты / PixiJS | **Отложить** | Графика не блокирует симуляцию. |
| Звук | **Отказ на сейчас** | Нет ценности для автоматического прогона. |
| Отдельная профессия «лекарь» | **Отложить** | Старцы + больная изба + семейный карантин покрывают механику. |
| Smoke 6480+ дней (9 лет) | **Следующий цикл** | Стресс после восьми лет. |
| Выбор размера карты из UI | **Отложить** | Seed уже есть (v1.6.0). |

## Следующие циклы (приоритет)

1. **Спрайты** — замена силуэтов.
2. **Smoke 6480 дней** — девять игровых лет.
3. **Вторная больная изба** при переполнении — если одна изба не вмещает все семьи.
...
**Идеи для следующего цикла:** спрайты; smoke 6480 дней; расширение больной избы при переполнении.
