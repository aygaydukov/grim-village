...
Обновлено: **2026-08-07** (ежедневный прогон Medieval Society Daily, v1.5.0).
...
|--------|---------|-------------|
| Smoke 2880 дней | **Сделано (v1.5.0)** | `MEGA_LONG_THRESHOLDS`, ≥4 рождений, стабильно seed=2026. |
| Визуал карантина на карте | **Сделано (v1.5.0)** | Фиолетовый туман, хижины, кольца жителей и старцев. |
| Seed в HUD | **Сделано (v1.5.0)** | Read-only отображение `world.seed` в шапке. |
...
| Выбор seed из UI (интерактив) | **Отложить** | Отображение seed есть; смена seed — после спрайтов. |
| Звук | **Отказ на сейчас** | Нет ценности для автоматического прогона. |
| Отдельная профессия «лекарь» | **Отложить** | Старцы уже покрывают механику. |

## Следующие циклы (приоритет)

1. **Интерактивный выбор seed** — перезапуск локального мира (не daemon).
2. **Path smoothing** — сглаживание углов A*.
3. **Спрайты** — замена силуэтов.
4. **Smoke 3600+ дней** — пять игровых лет, стресс миграции/иммиграции.
5. **Изоляция больных хижин** — механика, не только визуал.
...
| 2880 дней (v1.5.0) | MEGA_LONG, ≥4 рождений — **стабильно** |
| CI (`--ci`) 100 дней | 22+ живых, рождения | Стабильно по CI-порогам |

**Идеи для следующего цикла:** интерактивный seed; изоляция больных хижин; smoke 3600 дней.
