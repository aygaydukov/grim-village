...
Обновлено: **2026-08-10** (ежедневный прогон Medieval Society Daily, v1.8.0).
...
| Smoke 5040 дней | **Сделано (v1.8.0)** | `OMEGA_LONG_THRESHOLDS`, ≥7 рождений, стабильно seed=2026. |
| Path smoothing A* | **Сделано (v1.7.0)** | `smoothPath` + `hasLineOfSight` в `pathfind.ts`. |
| Smoke 4320 дней | **Сделано (v1.7.0)** | `HYPER_LONG_THRESHOLDS`, ≥6 рождений. |
| Изоляция больных хижин (по дому) | **Заменено (v1.8.0)** | Центральная больная изба вместо изоляции по семейным хижинам. |
| Спрайты / PixiJS | **Отложить** | Графика не блокирует симуляцию. |
| Звук | **Отказ на сейчас** | Нет ценности для автоматического прогона. |
| Отдельная профессия «лекарь» | **Отложить** | Старцы + больная изба покрывают механику. |
| Smoke 5760+ дней (8 лет) | **Следующий цикл** | Стресс после семи лет. |
| Выбор размера карты из UI | **Отложить** | Seed уже есть (v1.6.0). |

## Следующие циклы (приоритет)

1. **Спрайты** — замена силуэтов.
2. **Smoke 5760 дней** — восемь игровых лет.
3. **Перенос семей в больную избу** — сейчас идут только карантинные; дети/здоровые остаются дома.
...
|-------|-----------|-----------|
| 10 дней (v1.8.0) | smoke | **стабильно** |
| 100 дней CI (v1.8.0) | рождения, ≤ CI-порогов | **стабильно** (27 живых, 6 рождений) |
...
| 5040 дней (v1.8.0) | OMEGA_LONG, ≥7 рождений — **стабильно** |
| CI (`--ci`) 100 дней | 22+ живых, рождения | Стабильно по CI-порогам |

**Идеи для следующего цикла:** спрайты; smoke 5760 дней; расширение механики больной избы.

### Диагностика нестабильности

- Много смертей от **голода/холода** + пополнение **беженцами** → внутренний цикл слаб (см. инспектор «Деревня»).
- Много смертей от **болезни** без голода → эпидемия; соль, старцы и больная изба помогают.
